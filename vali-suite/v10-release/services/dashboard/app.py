"""
VALI Intelligence Dashboard v10
...
BLOCKLIST_FILE = Path("/app/blocklist/high_risk_ips.txt")
...
def load_blocklist():
    ...
    if BLOCKLIST_FILE.exists():
        for line in BLOCKLIST_FILE.read_text(encoding="utf-8").splitlines():
            ...
            if ip:
                entries.append({"ip": ip, "reason": reason})
    return entries
...
st.caption("Version 10.0 • Advanced deception fabric")
...
st.divider()
tab1, tab2, tab3, tab4 = st.tabs(["Sessions", "Campaigns", "Canaries", "Blocklist"])
...
with tab1:
    if not sessions:
        ...
        if choice:
            ...
            if sel.get("attack_path"):
                st.markdown("**Attack Path**")
                st.code(" → ".join(sel["attack_path"]))
            if sel["paths"]:
...
with tab4:
    bl = load_blocklist()
    if not bl:
        st.info("Blocklist empty. High-risk IPs and canary sources are added automatically by playbooks.")
    else:
        st.subheader("High-Risk IP Blocklist (local)")
        bdf = pd.DataFrame(bl)
        st.dataframe(bdf, use_container_width=True, hide_index=True)
        st.caption("These IPs crossed high-risk threshold or triggered canaries. Use downstream (firewall/WAF) as needed.")

st.divider()
st.caption("VALI v10 — Attackers spend. Defenders learn.")
