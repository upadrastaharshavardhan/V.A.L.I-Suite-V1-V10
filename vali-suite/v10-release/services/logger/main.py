"""
VALI Logger + Intelligence Engine v10
Scoring • Profiling • Campaigns • Kill-chain • Notes • Webhooks • Playbooks • Blocklist
"""
...
app = FastAPI(title="VALI Logger v10", version="10.0.0")

LOG_DIR = Path("/app/logs")
SESSION_DIR = Path("/app/sessions")
CANARY_DIR = Path("/app/canaries")
BLOCKLIST_DIR = Path("/app/blocklist")
CONFIG_PATH = Path("/app/config/vali.yaml")
for d in (LOG_DIR, SESSION_DIR, CANARY_DIR, BLOCKLIST_DIR):
    d.mkdir(parents=True, exist_ok=True)

API_KEY = os.getenv("LOGGER_API_KEY", "vali-logger-secret-key-change-me-v10")
WEBHOOK_URL = os.getenv("WEBHOOK_URL", "").strip()
AUTO_NOTE_ON_HIGH_RISK = os.getenv("AUTO_NOTE_ON_HIGH_RISK", "true").lower() == "true"
HIGH_RISK_THRESHOLD = int(os.getenv("HIGH_RISK_THRESHOLD", "60"))
WEBHOOK_ON_HIGH_RISK = os.getenv("WEBHOOK_ON_HIGH_RISK", "true").lower() == "true"
BLOCKLIST_FILE = BLOCKLIST_DIR / "high_risk_ips.txt"
...
def add_to_blocklist(ip: str, reason: str = ""):
    if not ip or ip in ("unknown", "127.0.0.1", "::1"):
        return
    existing = set()
    if BLOCKLIST_FILE.exists():
        existing = {line.split("#")[0].strip() for line in BLOCKLIST_FILE.read_text().splitlines() if line.strip()}
    if ip not in existing:
        with open(BLOCKLIST_FILE, "a", encoding="utf-8") as f:
            f.write(f"{ip}  # {reason} @ {utc_now()}\n")


def auto_note(session: dict, text: str, tag: str = "playbook"):
    session.setdefault("notes", []).append({"text": text, "tag": tag, "at": utc_now()})
...
def send_webhook_sync(payload: dict):
    if not WEBHOOK_URL:
        return
    try:
...
def update_session(session_id: str, event: dict):
    ...
    if path.exists():
        ...
        session = {
            ...
            "related_sessions": [], "notes": [], "kill_chain_tags": [], "attack_path": [],
        }
    ...
    prev_risk = session.get("risk_score", 0)
    session["risk_score"] = compute_risk(session)
    session["profile"] = profile_attacker(session)

    # Attack path summary (ordered unique kill-chain progression)
    ordered = []
    for e in session.get("events", []):
        for t in e.get("kill_chain", []):
            if t not in ordered and t not in ("general", "progress"):
                ordered.append(t)
    session["attack_path"] = ordered

    # Auto-playbooks
    if AUTO_NOTE_ON_HIGH_RISK and session["risk_score"] >= HIGH_RISK_THRESHOLD and prev_risk < HIGH_RISK_THRESHOLD:
        auto_note(session, f"Auto-playbook: session crossed high-risk threshold ({session['risk_score']}). Profile={session.get('profile')}.", "high_risk")
        add_to_blocklist(session.get("source_ip"), f"high_risk:{session['risk_score']}")
        if WEBHOOK_ON_HIGH_RISK:
            send_webhook_sync({
                "text": f"VALI High-Risk: IP `{session.get('source_ip')}` score={session['risk_score']} profile={session.get('profile')} path={' → '.join(session.get('attack_path') or [])}",
                "type": "high_risk",
    ...
    if event.get("event_type") == "canary":
        cid = (event.get("details") or {}).get("canary_id", "unknown")
        auto_note(session, f"Auto-playbook: canary triggered ({cid}).", "canary")
        add_to_blocklist(session.get("source_ip"), f"canary:{cid}")
...
async def campaigns(x_api_key: Optional[str] = Header(None)):
    ...
    for ip, sessions in by_ip.items():
        ...
        path_bits = []
        for s in sessions:
            for t in s.get("attack_path") or []:
                if t not in path_bits:
                    path_bits.append(t)
        narrative = (
            f"IP {ip} conducted {len(sessions)} session(s) across {', '.join(services) or 'unknown'} "
            f"with profiles [{', '.join(profiles)}]. "
            f"Max risk {max_risk}. "
            f"Kill-chain progression: {' → '.join(path_bits) if path_bits else 'limited activity'}. "
            f"{'Canary triggered. ' if canary_hits else ''}"
            f"{'High-value targeting observed.' if max_risk >= 60 else 'Activity under threshold.'}"
        )
        camps.append({
            "source_ip": ip,
            ...
            "campaign_risk": min(100, int(max_risk * 0.7 + (sum(risks)/len(risks) if risks else 0) * 0.3)),
            "profiles": profiles,
            "services": services,
            "canary_hits": canary_hits,
            "kill_chain_tags": sorted(tags),
            "attack_path": path_bits,
            "narrative": narrative,
...
@app.get("/export/stix-lite")
async def export_stix_lite(x_api_key: Optional[str] = Header(None)):
    """Simplified STIX-inspired bundle for threat intel sharing / SIEM ingest."""
    verify(x_api_key)
    objects = []
    for f in SESSION_DIR.glob("*.json"):
        try:
        ...
        # Indicator-like object
        objects.append({
            "type": "indicator",
            "id": f"indicator--vali-{sid[:12]}",
            "created": d.get("first_seen"),
            "modified": d.get("last_seen"),
            "name": f"VALI session {sid[:8]}",
            "description": (
                f"profile={d.get('profile')} risk={d.get('risk_score')} "
                f"path={' → '.join(d.get('attack_path') or [])} "
                f"tools={','.join(d.get('tools') or [])}"
            ),
            "pattern": f"[ipv4-addr:value = '{ip}']",
            "pattern_type": "stix",
            "valid_from": d.get("first_seen"),
            "labels": list(dict.fromkeys(
                (d.get("kill_chain_tags") or []) + [d.get("profile") or "unknown"]
            )),
            "x_vali": {
                "session_id": sid,
        ...
        objects.append({
            "type": "ipv4-addr",
            "id": f"ipv4-addr--vali-{ip.replace('.', '-')}",
            "value": ip,
        })
    return {
        "type": "bundle",
        "id": f"bundle--vali-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
        "objects": objects,
        "x_vali_version": "10.0.0",
    }
...
@app.get("/blocklist")
async def get_blocklist(x_api_key: Optional[str] = Header(None)):
    verify(x_api_key)
    ips = []
    if BLOCKLIST_FILE.exists():
    ...
    return {"count": len(ips), "entries": ips}


@app.get("/health")
async def health():
    return {"status": "ok", "service": "vali-logger", "version": "10.0.0"}
