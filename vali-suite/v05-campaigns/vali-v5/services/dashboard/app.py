"""
VALI Intelligence Dashboard v5
Risk • Profiles • Campaigns • Timelines • Canaries
"""
...
def build_campaigns(sessions):
    ...
    camps.sort(key=lambda x: x["max_risk"], reverse=True)
    return camps
...
st.caption("Version 5.0 • Advanced deception fabric")
...
c2.metric("Campaigns (IPs)", len(campaigns))
...
c5.metric("Canary Hits", sum(s["canary_hits"] for s in sessions) or len(canaries))
...
with tab1:
    if not sessions:
        st.info("No sessions yet. Use web decoy (:8080) or SSH (:2222).")
    else:
        ...
        choice = st.selectbox("Select session", list(opts.keys()), key="sess")
        if choice:
            sel = opts[choice]
            ...
            if sel["related_sessions"]:
                st.info("Related (same IP): " + ", ".join(
                    f"{r.get('service')}({r.get('risk_score')})" for r in sel["related_sessions"][:6]
                ))
            if sel["paths"]:
...
with tab2:
    if not campaigns:
        ...
        st.subheader("Attacker Campaigns (by Source IP)")
        cdf = pd.DataFrame([{
            "IP": c["source_ip"], "Sessions": c["sessions"],
            "Max Risk": c["max_risk"], "Avg Risk": c["avg_risk"],
...
st.divider()
st.caption("VALI v5 — Attackers spend. Defenders learn. Export via Logger API.")
