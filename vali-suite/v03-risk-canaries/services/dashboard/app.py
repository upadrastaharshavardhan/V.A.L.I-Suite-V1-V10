"""
VALI Intelligence Dashboard v3
Risk scores • Canaries • Full session intelligence
"""
...
st.caption("Version 3.0 • Advanced deception intelligence")
...
c5.metric("SSH Sessions", sum(1 for s in sessions if s["service"] == "ssh"))
c6.metric("Deep (≥3)", sum(1 for s in sessions if s["depth"] >= 3))

st.divider()

if not sessions:
    st.info("No sessions yet. Use the web decoy (8080) or SSH (2222) to generate intelligence.")
else:
    st.subheader("Sessions by Risk")
    df = pd.DataFrame([{
        "Risk": s["risk_score"],
    ...
    opts = {f"[{s['risk_score']}] {s['session_id']} | {s['source_ip']} | {s['service']}": s for s in sessions}
    ...
    if choice:
        ...
        with col2:
            ...
            st.write("**Login attempts:**", sel["login_attempts"])
        ...
        if sel["canaries_triggered"]:
            st.error("Canaries triggered: " + ", ".join(sel["canaries_triggered"]))

        if sel["paths"]:
            st.markdown("**Path Sequence**")
            st.code(" → ".join(sel["paths"][:30]))

        if sel["commands"]:
            st.markdown("**SSH Commands**")
            st.code("\n".join(sel["commands"][-25:]))

        if sel["user_agents"]:
            st.markdown("**User-Agents**")
            for ua in sel["user_agents"][:3]:
                st.code(ua)

st.divider()
if canaries:
    st.subheader("Recent Canary Alerts")
    cdf = pd.DataFrame([{
        "Time": (c.get("timestamp") or "")[:19],
        "Canary": (c.get("details") or {}).get("canary_id", "—"),
        "IP": c.get("source_ip"),
        "Session": (c.get("session_id") or "")[:12],
    } for c in canaries[-20:]])
    st.dataframe(cdf, use_container_width=True, hide_index=True)

st.divider()
st.caption("VALI v3 — Attackers spend. Defenders learn. Data stays local. Export via Logger API /export/sessions.json")
