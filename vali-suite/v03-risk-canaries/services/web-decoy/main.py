"""
VALI Web Decoy v3 — Progressive + Canaries + Adaptive Cost
"""
...
SECRET = os.getenv("WEB_DECOY_SECRET", "change-me-to-a-very-long-random-string-v3")
...
LOGGER_API_KEY = os.getenv("LOGGER_API_KEY", "vali-logger-secret-key-change-me-v3")
ENABLE_DELAY = os.getenv("ENABLE_ADAPTIVE_DELAY", "true").lower() == "true"
MIN_DELAY = int(os.getenv("MIN_DELAY_MS", "150")) / 1000.0
MAX_DELAY = int(os.getenv("MAX_DELAY_MS", "900")) / 1000.0
...
def load_unlocks():
    ...
    return {
        ...
        "staging": {"min_actions": 7, "requires": "api_docs"},
        "config_panel": {"min_actions": 11, "requires": "staging"},
        "backup_files": {"min_actions": 15, "requires": "config_panel"},
        "secrets_vault": {"min_actions": 18, "requires": "backup_files"},
    }
...
async def log_event(sid: str, etype: str, request: Request, path: str = None,
                    status: int = None, details: dict = None):
    payload = {
        "session_id": sid,
        "event_type": etype,
...
async def fire_canary(sid: str, canary_id: str, request: Request, extra: dict = None):
    try:
        async with httpx.AsyncClient(timeout=2.5) as c:
            await c.post(
                f"{LOGGER_URL}/canary",
...
def bump(request: Request, path: str):
...
async def process(request: Request, path: str, status: int = 200, details: dict = None):
    sid = get_sid(request)
    bump(request, path)
...
async def login_post(request: Request, username: str = Form(...), password: str = Form(...)):
    sid = await process(request, "/login", details={
        "username_attempt": username[:80], "password_length": len(password),
    })
    await log_event(sid, "login_attempt", request, path="/login",
                    details={"username": username[:80], "success": False})
...
async def vault(request: Request):
    ...
    # Touching the vault is high-value — treat as soft canary
    await fire_canary(sid, "canary-vault-access", request)
...
async def api_secrets(request: Request):
    ...
    return JSONResponse({"status": "error", "message": "Forbidden", "code": "FORBIDDEN"}, status_code=403)
...
async def backup_download(request: Request, name: str):
    ...
    return JSONResponse({"status": "error", "message": "Authorization required for download"}, status_code=401)
...
async def api_health(request: Request):
    ...
    return {"status": "ok", "version": "3.1.0-internal", "env": "production"}
